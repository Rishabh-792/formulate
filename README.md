...
**5/5 exact, max absolute error 0.00e+00.** Median end-to-end pipeline
latency is 13 ms on the CI runner (Linux, Python 3.12) and 37 ms on a Windows
laptop — quote the number with its platform, since the two differ by 3x. Full
results, including per-problem timings and the transforms each one triggered,
are committed to [`bench/results.json`](bench/results.json) and regenerated by
CI, which fails if any problem misses its enumerated optimum.
